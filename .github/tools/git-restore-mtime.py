...
def normalize(path):
    ...
    if path and path[0] == '"':
        ...
        path = (path[1:-1]                 # Remove enclosing double quotes
                # Convert to bytes, required by 'unicode-escape'
                .encode('latin1')
                # Perform the actual octal-escaping decode
                .decode('unicode-escape')
                # 1:1 mapping to bytes, forming UTF-8 encoding
                .encode('latin1')
...
class Git:
    ...
    def _run(self, cmdstr: str, paths: list = None, output=True, check=False):
        ...
        self._proc = subprocess.Popen(
            cmdlist, stdout=subprocess.PIPE, **popen_args)
        try:
...
def main():
    ...
    log.info("{0:,} files to be processed in work dir".format(
        stats['totalfiles']))
